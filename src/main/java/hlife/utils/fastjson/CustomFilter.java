...
public class CustomFilter implements PropertyPreFilter {
	...
	private Map<Class<?>, String[]> includes = new HashMap<Class<?>, String[]>(); // 需要序列化的属性
	private Map<Class<?>, String[]> excludes = new HashMap<Class<?>, String[]>(); // 不需要序列化的属性
	...
	private boolean isNeed(Class<?> clazz, Map<Class<?>, String[]> map, String name) {
		...
		for (Map.Entry<Class<?>, String[]> item : map.entrySet()) {
			/*
			 * isAssignableFrom(): 判定此 Class 对象所表示的类或接口与指定的 Class 参数所表示的类或接口是否相同，
			 * 或是否是其超类或超接口。
			 */
